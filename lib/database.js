...
export const useUser = (defaultUser) => {
  ...
  useEffect(() => {
    if (personalData && !loading) {
      const mail = personalData.mail
      console.log(personalData)

      const personal = Object.fromEntries(Object.entries(personalData).filter(([k, v]) => k != "mail"));
      const payload = {personalData: personal, date: Date.now().toString()}
